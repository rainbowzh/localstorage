...
(function(global, namespace, factory) {
  ...
  const LocalCache = {
    ...
    _setCacheFormat: function(name, value, time, other) {
      ...
      switch (LEN) {
        case 1:
          if (this._dataTypeCheck(name) === "array") {
            //对象数组存储
        ...
        case 2:
          if (
            this._dataTypeCheck(name) === "string" &&
            this._dataTypeCheck(value) === "string"
          ) {
            let _key = name;
            let _value = value;
            let cacheValue = {
        ...
        case 3:
          if (
            this._dataTypeCheck(name) === "string" &&
            this._dataTypeCheck(value) === "string" &&
            this._dataTypeCheck(time) === "number"
          ) {
            let _key = name;
            let _value = value;
            let _time = time;
    ...
    get: function(name) {
      ...
      if (this._isSupport && name && originKey) {
        ...
        console.log("不支持localStorage存储或数据被手动删除");
      }
    },
    ...
    expired: function(value) {
      ...
      if (value&&originKey) {
        ...
        if (item&&item.value.time > new Date()) {
          return false; //未过期
        } else {
          return true;
        }
